...
async function togglePopup(item: ExtendedSearchResult, event: MouseEvent) {
  try {
    ...
    if (ret === undefined) {
      // create item
      // threshold x location coords
      console.log("prev x:", event.clientX - 2);
      console.log(
        "new x:",
        Math.min(event.clientX - 2, window.innerWidth * 0.74 - 125)
      );
      await db.popups.add({
        id: 1,
        ...
        display: false,
        location: {
          x: Math.min(event.clientX - 2, window.innerWidth * 0.74 - 250),
          y: event.clientY + 20,
        },
      ...
      await db.popups.update(1, {
        ...
        display: false,
        location: {
          x: Math.min(event.clientX - 2, window.innerWidth * 0.72 - 300),
          y: event.clientY + 20,
        },
        triggerword: item.triggerword,
        wordlocation: item.range,
...
async function loadSidebar(item: ExtendedSearchResult) {
  try {
    ...
    console.log("loading sidebar with item:", item);
    if (ret === undefined) {
...
// If need to align things to side of editor, will have to modify this.
const cursorTooltipBaseTheme = EditorView.baseTheme({
  ".cm-tooltip.cm-tooltip-cursor": {
    ...
    border: "none",
    padding: "4px 4px",
    // borderRadius: "6px", // for circle vs square
    height: "6px",
    cursor: "pointer",
    position: "relative",
    // marginRight: "8px",
